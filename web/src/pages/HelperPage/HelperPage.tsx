...
const reverseTransformCharacter = ($Hc) => {
  ...
  let transformedChar = String.fromCharCode(_$t$_)
  if (transformedChar === '6') {
    transformedChar = '@'
  }
...
const HelperPage = () => {
  ...
  const reverseHardcodedString = () => {
    const r1 = h1.split('').map(reverseTransformCharacter).join('')
  ...
  const r2 = () => {
    const reversed = h2.split('').map(reverseTransformCharacter).join('')
    setRv2(reversed)
  }
  const decodeString = () => {
